...
(function( $, window, document, undefined ){
    ...
    Picker.prototype = {
        ...
        pc_remove: function(e){
            var $elem = $(e.target);
            var selectedText = $elem.parent().text();
            var order = $elem.parent().data('order');

            var li = $("<li>").html($elem.parent().text()).attr('data-order', order);
            ...
            this.$elem.find("option").filter(function() {return $(this).text() == selectedText;}).removeAttr("selected");
            $elem.parent().remove();
            this.$elem.trigger('sp-change');
        },
        ...
        _selectElement: function (id, $elem) {
            ...
            if($elem == undefined){
                ...
                $elem = this.$container.find('.pc-list li[text="' + id + '"]');
            ...
            this.$elem.find("option").filter(function() {return $(this).text() == id;}).attr("selected", "selected");

        },

        _insertIntoCurrentData: function (elem) {
            var $elem = $(elem.target);
            var selectedId = $elem.parent().text();
            var order = $elem.parent().data('order');
        ...
        _createElement: function($elem){
            var tagClass = this.config.coloring[$elem.text()];
            var root = $("<span>").addClass("pc-element " + (tagClass ? tagClass : "")).text($elem.text())
                .attr('data-order',$elem.data('order'));
        ...
        _fillList: function(){
            ...
            this.$elem.find('option:not([hidden])').each(function(index, elem){
                var li = $("<li>").html($(elem).text()).attr('data-order', counter);
                li.click(this.pc_selected.bind(this));
        ...
        _updateList: function(filteredData, searchedValue){
            ...
            for(i = 0; i < filteredData.length; i++){
                ...
                var li = $("<li>").html(liContent).attr('data-order', filteredData[i].order);
                li.click(this.pc_selected.bind(this));
                listContainer.append(li);
            }
        }
